...
func (r *CartPostgres) AddToCart(cartItem *entities.CartItem) (int64, error) {
	...
	existsQuery := `SELECT cart_item_id FROM cart_item WHERE volume_id = $1 AND user_id = $2 AND is_cold = $3 and is_hot = $4`
	err := r.db.Get(&cartItemID, existsQuery, cartItem.Volume.ID, cartItem.UserID, cartItem.IsCold, cartItem.IsHot)
	if err == nil {
    ...
    (volume_id, user_id, is_hot, is_cold, quantity) VALUES ($1, $2, $3, $4, $5) RETURNING cart_item_id`

	err = r.db.QueryRowx(query, cartItem.Volume.ID, cartItem.UserID, cartItem.IsHot, cartItem.IsCold, cartItem.Quantity).Scan(&cartItemID)
	if err != nil {
...
func (r *CartPostgres) GetCart(userID int64) ([]*entities.CartItem, error) {
...
func (r *CartPostgres) DeleteCartItem(cartItemID int64, userID int64) error {
